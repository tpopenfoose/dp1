...
#    plot.new()
    ...
    par(mar=c(2,10,8,2))
    chart.CumReturns.gm(x, main = main, xaxis = FALSE, legend.loc = legend.loc, event.labels = event.labels, ylog = ylog, wealth.index = wealth.index, begin = begin, geometric = geometric, ylab="", ...)
    title(ylab="Gain/Loss (%)", line = 5)
    ...
    chart.BarVaR(x*100, main = "", xaxis = FALSE, width = width, ylab = "", methods = methods, event.labels = NULL, ylog=FALSE, gap = gap, p=p, ...)
    title(ylab=paste(date.label,"\nReturn (%)"), line = 5)

    # The third row is the underwater plot
    par(mar=c(5,10,0,2))
    chart.Drawdown.gm(x, geometric = geometric, main = "", ylab = "", event.labels = NULL, ylog=FALSE, ...)
    title(ylab="Drawdown (%)", line = 5)
